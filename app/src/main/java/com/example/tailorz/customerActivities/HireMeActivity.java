...
public class HireMeActivity extends AppCompatActivity {
    ...
    private void placeOrder() {
        binding.btnConfirmOrder.setOnClickListener(v -> {
            ...
            // ✅ Get logged-in User ID and Customer Name from SharedPreferences
            final String customerID = prefs.getUserID();
            final String customerName = prefs.getUserName();
            final String tailorNameFromIntent = tailorName; // ✅ Make it final
            ...
            if (tailorNameFromIntent == null || tailorNameFromIntent.isEmpty()) {
                Toast.makeText(this, "Error: Tailor name not found! Please try again.", Toast.LENGTH_LONG).show();
                binding.btnConfirmOrder.revertAnimation();
                return;
            }
            ...
            customerRef.addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    if (snapshot.exists()) {
                        String customerEmail = snapshot.child("email").getValue(String.class);
                        if (customerEmail == null || customerEmail.isEmpty()) {
                            customerEmail = "No Email";
                        }

                        // ✅ Ensure Tailor Name is Correct
                        String finalTailorName = (tailorNameFromIntent == null || tailorNameFromIntent.isEmpty()) ? "Unknown Tailor" : tailorNameFromIntent;

                        // ✅ Ensure Design Name is Not Null
                        String finalDesignName = (designName == null || designName.isEmpty()) ? "Unknown Design" : designName;

                        // ✅ Create Order Model
                        NotificationModel order = new NotificationModel();
                        order.setOrderID(orderID);
                        order.setDesignName(finalDesignName);
                        order.setCustomerName(customerName); // ✅ Now properly set
                        order.setTailorName(finalTailorName); // ✅ Now properly set
                        order.setDate(currentDate);
                        order.setTime(currentTime);
                        order.setStatusValue(1); // 1 = Pending

                        // ✅ Log Order Details for Debugging
                        Log.d("HireMeActivity", "Saving Order:");
                        Log.d("HireMeActivity", "Order ID: " + orderID);
                        Log.d("HireMeActivity", "Customer Name: " + order.getCustomerName());
                        Log.d("HireMeActivity", "Tailor Name: " + order.getTailorName());
                        Log.d("HireMeActivity", "Design Name: " + order.getDesignName());
                        Log.d("HireMeActivity", "Date: " + order.getDate());
                        Log.d("HireMeActivity", "Time: " + order.getTime());
                        Log.d("HireMeActivity", "Status: " + order.getStatusValue());
